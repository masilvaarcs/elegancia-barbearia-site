import { eleganciaData, resolveClientPlan } from "../../app/lib/elegancia-data";

describe("eleganciaData", () => {
  it("should expose public business contact info", () => {
    expect(eleganciaData.brand.name).toBe("Elegância Barbearia");
    expect(eleganciaData.brand.phoneRaw).toMatch(/^55\d{10,11}$/);
    expect(eleganciaData.brand.whatsappUrl).toContain(`wa.me/${eleganciaData.brand.phoneRaw}`);
    expect(eleganciaData.brand.instagramHandle).toContain("masilva_arcs");
  });

  it("should include at least five gallery images", () => {
    expect(eleganciaData.gallery.length).toBeGreaterThanOrEqual(5);
    const sources = eleganciaData.gallery.map((item) => item.src);
    expect(new Set(sources).size).toBe(sources.length);
  });

  it("should define complete navigation anchors", () => {
    const hrefs = eleganciaData.nav.map((item) => item.href);
